...
cat /opt/cc/aws-install-config.yaml.template | sed "s/pullSecret: ''/pullSecret: '$PULL_SECRET_CONTENTS'/"| sed "s/name: ''/name: $CLUSTER_NAME/" > $CLUSTER_DIR/aws/$CLUSTER_NAME/install-config.yaml
